#!/usr/bin/env python3
"""Build Phase 134 focused review for the Matbench log GVRH gate.

This phase consumes only small Phase 129 artifacts. It checks whether the
Phase 129 shear-modulus gap is stable under alternate grouped/binned splits and
...
DEFAULT_PHASE129_DIR = Path("docs/results/phase129_matbench_log_gvrh_baseline_gate")
DEFAULT_OUTPUT_DIR = Path("docs/results/phase134_matbench_log_gvrh_focused_review")
MIN_SPLIT_ROWS = 100
...
phase129 = _load_module("build_phase129_matbench_log_gvrh_baseline_gate.py", "phase129_helpers")
...
PROFILE_COLUMNS: dict[str, dict[str, Any]] = {
    "composition_descriptors": {
        **phase129.PROFILE_COLUMNS["composition_descriptors"],
        "role": "admissible",
    },
    "common_element_fractions": {
        **phase129.PROFILE_COLUMNS["common_element_fractions"],
        "role": "admissible",
    },
    "lattice_descriptors": {
        **phase129.PROFILE_COLUMNS["lattice_descriptors"],
        "role": "admissible",
    },
    "composition_lattice_descriptors": {
        **phase129.PROFILE_COLUMNS["composition_lattice_descriptors"],
        "role": "admissible",
    },
    "composition_hash_shortcut": {
        **phase129.PROFILE_COLUMNS["composition_hash_shortcut"],
        "role": "negative_control",
    },
    "chemistry_family_shortcut": {
        **phase129.PROFILE_COLUMNS["chemistry_family_shortcut"],
        "role": "negative_control",
    },
...
MODEL_METHODS = phase129.MODEL_METHODS
PROFILE_METHODS = {
...
NN_FEATURE_COLUMNS = tuple(
    column
    for column in (
        *(f"frac_{element}" for element in phase129.ELEMENTS),
        "n_sites",
        "lattice_a",
        "lattice_b",
        "lattice_c",
...
SPLIT_PLAN = (
    ("phase129_registered_split", "phase129_manifest", "phase129"),
    ("chemistry_family_hash_0", "group:chemistry_family_key", "phase134_family_0"),
    ("chemistry_family_hash_1", "group:chemistry_family_key", "phase134_family_1"),
    ("chemistry_family_hash_2", "group:chemistry_family_key", "phase134_family_2"),
    ("dominant_element_hash", "group:dominant_element", "phase134_dominant"),
    ("lattice_volume_bins", "bins:lattice_volume", "phase134_lattice_volume"),
    ("volume_per_site_bins", "bins:volume_per_site", "phase134_volume_per_site"),
    ("element_count_bins", "bins:element_count", "phase134_element_count"),
    ("density_anisotropy_bins", "bins:density_z_proxy,abc_anisotropy", "phase134_density_aniso"),
)
...
def _registered_split(split_manifest: dict[str, Any]) -> dict[str, Any]:
    return {
        "split_strategy": split_manifest.get("split_strategy", "phase129_registered_split"),
        "split_salt": "phase129",
        "split_viable": True,
        "reason": "ok",
...
def build_split_reviews(df: pd.DataFrame, split_manifest: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    for split_id, split_spec, salt in SPLIT_PLAN:
        if split_spec == "phase129_manifest":
...
def build_audit_rows(*, phase129_gate: dict[str, Any], split_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    phase129_ready = phase129_gate.get("status") == "phase129_matbench_log_gvrh_ready_focused_review"
    rows.append(
        _audit_row(
            audit="phase129_gate_status",
            status="pass" if phase129_ready else "block",
            severity="blocking" if not phase129_ready else "info",
            value=phase129_gate.get("status"),
            threshold="phase129_matbench_log_gvrh_ready_focused_review",
            reason="focused review requires a Phase 129 focused-review gate",
        )
    )
    original = next((row for row in split_rows if row.get("split_id") == "phase129_registered_split"), None)
    if original and original.get("split_viable"):
        rows.append(
            _audit_row(
                ...
                reason="Phase 129 selected GVRH target must preserve validation and test gain",
            )
        )
        rows.append(
            _audit_row(
        ...
        rows.append(
            _audit_row(
                ...
                reason="Phase 129 registered split must be reviewable",
            )
        )
    ...
    rows.append(
        _audit_row(
            ...
            reason="GVRH target gain must survive deterministic chemistry, dominant-element, and lattice split perturbations",
        )
    )
    ...
    rows.append(
        _audit_row(
            ...
            threshold=0,
            reason="no viable split may have severe train/validation/test GVRH target mean, median, or q90 imbalance",
        )
    )
    return rows


def build_gate(*, phase129_gate: dict[str, Any], split_rows: list[dict[str, Any]], audit_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    original = next((row for row in split_rows if row.get("split_id") == "phase129_registered_split"), {})
    if phase129_gate.get("status") != "phase129_matbench_log_gvrh_ready_focused_review":
        status = "phase134_matbench_log_gvrh_review_blocked_by_phase129"
        mechanism_allowed = False
        next_action = "complete or close Phase 129 before focused review"
    elif blockers:
        status = "phase134_matbench_log_gvrh_focused_review_closed_split_sensitivity_or_shortcut"
        mechanism_allowed = False
        next_action = "close the Phase 129 GVRH target as diagnostic; do not train"
    else:
        status = "phase134_matbench_log_gvrh_focused_review_ready_low_capacity_mechanism_gate"
        mechanism_allowed = True
        next_action = "design a separate no-training low-capacity GVRH mechanism gate; keep model training closed"
    return {
        "status": status,
        "phase129_status": phase129_gate.get("status"),
        "selected_target": phase129_gate.get("selected_target", "log10_g_vrh"),
        "phase129_selected_profile": phase129_gate.get("selected_profile"),
        "phase129_selected_method": phase129_gate.get("selected_method"),
        ...
        "phase134_model_mechanism_allowed": mechanism_allowed,
        "phase134_low_capacity_mechanism_design_allowed": mechanism_allowed,
        "phase134_model_training_allowed": False,
...
def build_markdown(gate: dict[str, Any], audit_rows: list[dict[str, Any]], split_rows: list[dict[str, Any]]) -> str:
    ...
    lines = [
        ...
        f"- Low-capacity mechanism design allowed: `{gate['phase134_low_capacity_mechanism_design_allowed']}`",
        f"- Model training allowed: `{gate['phase134_model_training_allowed']}`",
        "",
        "## Blocking Audits",
        "",
...
def build_package(*, root: Path, phase129_dir: Path, output_dir: Path) -> dict[str, Any]:
    _configure_phase127_helpers()
    field_path = phase129_dir / "phase129_matbench_log_gvrh_field_table.csv"
    split_path = phase129_dir / "phase129_matbench_log_gvrh_split_manifest.json"
    gate_path = phase129_dir / "phase129_matbench_log_gvrh_gate.json"
    df = pd.read_csv(field_path)
    split_manifest = _read_json(split_path)
    phase129_gate = _read_json(gate_path)
    target = str(phase129_gate.get("selected_target") or "log10_g_vrh")
    ...
    audit_rows = build_audit_rows(phase129_gate=phase129_gate, split_rows=split_rows)
    gate = build_gate(phase129_gate=phase129_gate, split_rows=split_rows, audit_rows=audit_rows)

    output_dir.mkdir(parents=True, exist_ok=True)
    profile_path = output_dir / "phase134_matbench_log_gvrh_focused_profile_table.csv"
    split_review_path = output_dir / "phase134_matbench_log_gvrh_split_sensitivity_table.csv"
    audit_path = output_dir / "phase134_matbench_log_gvrh_shortcut_audit_table.csv"
    split_manifest_path = output_dir / "phase134_matbench_log_gvrh_split_review_manifest.json"
    gate_out = output_dir / "phase134_matbench_log_gvrh_focused_review_gate.json"
    markdown_path = output_dir / "phase134_matbench_log_gvrh_focused_review.md"
    manifest_path = output_dir / "phase134_matbench_log_gvrh_focused_review_manifest.json"
    ...
    manifest = {
        "phase": 134,
        "objective": "matbench_log_gvrh_focused_split_shortcut_review_no_training",
        "inputs": {
            "phase129_dir": phase127._display_path(phase129_dir, root),
            "field_table": phase127._display_path(field_path, root),
            "split_manifest": phase127._display_path(split_path, root),
            "phase129_gate": phase127._display_path(gate_path, root),
        },
        "outputs": {
...
def main() -> None:
    ...
    phase129_dir = args.phase129_dir if args.phase129_dir.is_absolute() else root / args.phase129_dir
    output_dir = args.output_dir if args.output_dir.is_absolute() else root / args.output_dir
    manifest = build_package(root=root, phase129_dir=phase129_dir, output_dir=output_dir)
