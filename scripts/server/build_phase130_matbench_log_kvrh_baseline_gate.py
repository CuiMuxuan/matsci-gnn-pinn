#!/usr/bin/env python3
"""Build a Phase 130 baseline-first gate for Matbench log KVRH.

This phase opens a fresh small public external data-source intake after the
Phase 129 log GVRH baseline-first gate. It downloads Matbench v0.1
``matbench_log_kvrh`` if needed, parses structure JSON dictionaries into
composition and lattice descriptors, and reviews the bulk-modulus target
``log10(K_VRH)`` with strong tabular baselines and shortcut controls. It does
...
SOURCE_URL = "https://ml.materialsproject.org/projects/matbench_log_kvrh.json.gz"
DEFAULT_RAW_PATH = Path("data/raw/external/matbench_log_kvrh/matbench_log_kvrh.json.gz")
DEFAULT_OUTPUT_DIR = Path("docs/results/phase130_matbench_log_kvrh_baseline_gate")
EXPECTED_MIN_BYTES = 1_000_000
EXPECTED_HEAD_BYTES = 4_174_387
...
def ensure_source_file(path: Path, *, source_url: str, force_download: bool = False) -> dict[str, Any]:
    ...
    if force_download or not path.exists() or path.stat().st_size < EXPECTED_MIN_BYTES:
        request = urllib.request.Request(
            source_url,
            headers={"User-Agent": "Mozilla/5.0 Codex Phase130 matbench log_kvrh gate"},
        )
...
def load_matbench_payload(path: Path) -> tuple[list[str], list[list[Any]]]:
    ...
    if columns != ["structure", "log10(K_VRH)"]:
        raise ValueError(f"Unexpected matbench_log_kvrh columns: {columns}")
...
def build_field_table(rows: list[list[Any]]) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    for index, row in enumerate(rows):
        ...
        record = {
            "phase130_row_id": f"P130-{index:05d}",
            "composition": composition,
            "log10_k_vrh": float(target),
...
def group_split(df: pd.DataFrame, *, group_column: str = "chemistry_family_key", salt: str = "phase130") -> dict[str, Any]:
...
def build_schema_rows(df: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    for column in df.columns:
        ...
        role = "feature"
        if column == "log10_k_vrh":
            role = "target"
        elif column in {"phase130_row_id", "composition"}:
            role = "identifier"
...
def build_gate(*, source_info: dict[str, Any], split_manifest: dict[str, Any], review: dict[str, Any]) -> dict[str, Any]:
    status = (
        "phase130_matbench_log_kvrh_ready_focused_review"
        if review.get("phase130_candidate")
    ...
    return {
        ...
        "phase130_focused_review_allowed": bool(review.get("phase130_candidate")),
...
def build_markdown(gate: dict[str, Any], review: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            f"- Focused review allowed: `{gate['phase130_focused_review_allowed']}`",
            f"- Model training allowed: `{gate['phase130_model_training_allowed']}`",
...
def build_package(
    *,
    ...
    field_path = output_dir / "phase130_matbench_log_kvrh_field_table.csv"
    split_path = output_dir / "phase130_matbench_log_kvrh_split_manifest.json"
    metric_path = output_dir / "phase130_matbench_log_kvrh_metric_table.csv"
    review_path = output_dir / "phase130_matbench_log_kvrh_target_review_table.csv"
    schema_path = output_dir / "phase130_matbench_log_kvrh_schema_table.csv"
    data_card_path = output_dir / "phase130_matbench_log_kvrh_data_card.json"
    gate_path = output_dir / "phase130_matbench_log_kvrh_gate.json"
    markdown_path = output_dir / "phase130_matbench_log_kvrh.md"
    manifest_path = output_dir / "phase130_matbench_log_kvrh_manifest.json"
    ...
    manifest = {
        "phase": 130,
        "objective": "matbench_log_kvrh_baseline_first_gate_no_training",
        "source": source_info,
