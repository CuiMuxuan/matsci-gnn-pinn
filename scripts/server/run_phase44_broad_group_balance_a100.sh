...
DATA_LOSS_GROUP_BALANCE_COLUMN="${DATA_LOSS_GROUP_BALANCE_COLUMN:-process_condition}"
DATA_LOSS_GROUP_BALANCE_STRENGTH="${DATA_LOSS_GROUP_BALANCE_STRENGTH:-1.0}"
PROCESS_FEATURE_TAG="${PROCESS_FEATURE_TAG:-group_bal}"

cd "$REPO_ROOT"
mkdir -p logs

run_group_balance_holdout() {
  ...
  echo "=== ${run_id} group_balance=${DATA_LOSS_GROUP_BALANCE_COLUMN}@${DATA_LOSS_GROUP_BALANCE_STRENGTH} ==="
  RUN_ID="$run_id" \
  ...
  DATA_LOSS_GROUP_BALANCE_COLUMN="$DATA_LOSS_GROUP_BALANCE_COLUMN" \
  DATA_LOSS_GROUP_BALANCE_STRENGTH="$DATA_LOSS_GROUP_BALANCE_STRENGTH" \
...
for dataset_limit in $DATASET_LIMITS; do
  for split_strategy in $PROFILE_SPLITS; do
    case "$split_strategy" in
      laser_power)
        run_group_balance_holdout "$dataset_limit" laser_power 0.34 0.33 0.33
        ;;
      spot_size)
        run_group_balance_holdout "$dataset_limit" spot_size 0.34 0.33 0.33
        ;;
      scan_speed)
        run_group_balance_holdout "$dataset_limit" scan_speed 0.34 0.33 0.33
        ;;
      line)
        run_group_balance_holdout "$dataset_limit" line 0.6 0.2 0.2
        ;;
      process)
        run_group_balance_holdout "$dataset_limit" process 0.6 0.2 0.2
        ;;
      *)
