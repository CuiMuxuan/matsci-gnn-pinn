#!/usr/bin/env python3
"""Build Phase 156 focused review for the UCI superconductivity gate.

This phase audits whether the Phase 155 UCI critical-temperature baseline gap
survives alternate leakage-safe element-set splits, shortcut controls, nearest
neighbor identity controls, and target-distribution shifts. It may only open a
later no-training low-capacity mechanism gate; it must not train a neural model.
"""
...
DEFAULT_PHASE155_DIR = Path("docs/results/phase155_uci_superconductivity_baseline_gate")
DEFAULT_RAW_PATH = Path(
    "data/raw/external/phase155_uci_superconductivity/superconductivty_data.zip"
)
DEFAULT_OUTPUT_DIR = Path("docs/results/phase156_uci_superconductivity_focused_review")
...
SPLIT_SPECS = (
    ...
    ("element_set_hash_1", "element_set_hash", "phase156_element_set_1", True),
    ("element_set_hash_2", "element_set_hash", "phase156_element_set_2", True),
    ("element_set_hash_3", "element_set_hash", "phase156_element_set_3", True),
    ("element_set_hash_4", "element_set_hash", "phase156_element_set_4", True),
    ("dominant_element_holdout", "group:dominant_element", "phase156_dominant", False),
    ("number_of_elements_bins", "bins:number_of_elements", "phase156_number_elements", False),
    ("max_fraction_bins", "bins:max_element_fraction", "phase156_max_fraction", False),
)
...
def _load_phase155_module():
    script = Path(__file__).with_name("build_phase155_uci_superconductivity_baseline_gate.py")
    spec = importlib.util.spec_from_file_location("phase155_helpers", script)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Could not load Phase 155 helper module from {script}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


p155 = _load_phase155_module()
...
def _split_from_keys(keys: list[str], *, salt: str, strategy: str) -> dict[str, Any]:
    ...
    for group in groups:
        value = p155._stable_unit_hash(group, salt)
        if value < 0.60:
...
def _element_set_split(df: pd.DataFrame, salt: str) -> dict[str, Any]:
    keys = df["element_set_key"].fillna("unknown").astype(str).tolist()
    return _split_from_keys(keys, salt=salt, strategy="group_hash_by_element_set_key")
...
def _build_split_info(df: pd.DataFrame, spec: tuple[str, str, str, bool]) -> dict[str, Any]:
    ...
    if strategy == "element_set_hash":
        info = _element_set_split(df, salt)
...
def _nearest_neighbor_control(df: pd.DataFrame, assignments: list[str]) -> dict[str, Any]:
    y = df["target_critical_temp_K"].to_numpy(dtype=float)
    columns = list(p155.profile_columns(df)["element_fraction_vector"]["columns"])
...
def _review_for_split(
    ...
    if not split_info.get("split_viable"):
        return [], {**base, "phase156_split_pass": False}, split_info

    metric_rows, review_rows = p155.evaluate_baselines(df, split_info["assignments"])
    ...
    row = {
        ...
        **nearest,
        ...
        "phase156_split_pass": split_pass,
    }
...
def build_audit_rows(*, phase155_gate: dict[str, Any], split_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    gate_ready = (
        phase155_gate.get("status") == "phase155_uci_superconductivity_ready_focused_review"
        and bool(phase155_gate.get("phase156_focused_review_allowed"))
    )
    rows.append(
        _audit_row(
            audit_id="P156-AUDIT-001",
            audit="phase155_gate_consistency",
            status="pass" if gate_ready else "block",
            severity="blocking" if not gate_ready else "info",
            value=phase155_gate.get("status"),
            threshold="phase155_uci_superconductivity_ready_focused_review",
            reason="Phase 156 requires a Phase 155 baseline-first gate that allowed focused review",
        )
    )
    gate_rows = [row for row in split_rows if row.get("gate_review_split") and row.get("split_viable")]
    passed = [row for row in gate_rows if row.get("phase156_split_pass")]
    stable_rate = float(len(passed) / len(gate_rows)) if gate_rows else 0.0
    rows.append(
        _audit_row(
            audit_id="P156-AUDIT-002",
            audit="stable_element_set_split_pass_rate",
            ...
            reason="baseline-visible signal should survive most leakage-safe element-set splits",
        )
    )
    registered = next(
        (row for row in split_rows if row.get("split_id") == "phase155_registered_element_set"),
        None,
    )
    registered_pass = bool(registered and registered.get("phase156_split_pass"))
    rows.append(
        _audit_row(
            audit_id="P156-AUDIT-003",
            audit="registered_split_replay",
            status="pass" if registered_pass else "block",
            severity="blocking" if not registered_pass else "info",
            value=registered.get("phase156_split_pass") if registered else None,
            threshold=True,
            reason="the Phase 155 registered split must replay as a guarded candidate",
        )
    )
    shortcut_dominant = [row for row in gate_rows if row.get("shortcut_dominant")]
    rows.append(
        _audit_row(
            audit_id="P156-AUDIT-004",
            ...
            threshold=0,
            reason="formula-shape, element-set hash, or row-order controls must not dominate",
        )
    )
    nearest_dominant = [row for row in gate_rows if row.get("nearest_neighbor_dominant")]
    rows.append(
        _audit_row(
            audit_id="P156-AUDIT-005",
            ...
            threshold=0,
            reason="nearest-neighbor composition identity control must not explain the signal",
        )
    )
    shifted = [
        row
...
def build_gate(
    *,
    ...
    registered = next(row for row in split_rows if row["split_id"] == "phase155_registered_element_set")
    return {
        "status": status,
        "source": phase155_gate.get("source"),
        "source_doi": phase155_gate.get("source_doi"),
        "selected_target": phase155_gate.get("selected_target"),
        "phase155_selected_profile": phase155_gate.get("selected_profile"),
        "phase155_selected_method": phase155_gate.get("selected_method"),
        "phase155_validation_rmse": phase155_gate.get("selected_validation_rmse"),
        "phase155_test_rmse": phase155_gate.get("selected_test_rmse"),
        ...
        "phase156_model_mechanism_allowed": mechanism_allowed,
        "phase156_model_training_allowed": False,
        "a100_training_allowed_now": False,
        "a100_80gb_request_now": False,
        "next_action": (
            "run Phase 157 no-training low-capacity mechanism gate before any neural training"
            if mechanism_allowed
            else "close Phase 155/156 as diagnostic or choose another second-paper source"
        ),
    }
...
def build_markdown(
    *,
    ...
    lines = [
        ...
        f"- Model mechanism allowed: `{_csv_value(gate['phase156_model_mechanism_allowed'])}`",
        f"- Model training allowed: `{_csv_value(gate['phase156_model_training_allowed'])}`",
        ...
        "## Interpretation",
        (
            "This focused review checks whether the Phase 155 source gate is stable enough "
            "to justify a later no-training low-capacity mechanism gate. It does not "
            "train a neural model or support a second-paper model claim by itself."
        ),
        "",
        "## Split Review",
...
def build_package(
    *,
    ...
    root = root.resolve()
    phase155_dir = phase155_dir if phase155_dir.is_absolute() else root / phase155_dir
    raw_path = raw_path if raw_path.is_absolute() else root / raw_path
    output_dir = output_dir if output_dir.is_absolute() else root / output_dir

    phase155_gate = _read_json(phase155_dir / "phase155_uci_superconductivity_baseline_gate.json")
    if not raw_path.exists():
        manifest_path = phase155_dir / "phase155_uci_superconductivity_baseline_manifest.json"
    ...
    audit_rows = build_audit_rows(phase155_gate=phase155_gate, split_rows=split_rows)
    gate = build_gate(phase155_gate=phase155_gate, split_rows=split_rows, audit_rows=audit_rows)

    split_table_path = output_dir / "phase156_uci_superconductivity_split_review_table.csv"
    metric_table_path = output_dir / "phase156_uci_superconductivity_metric_table.csv"
    audit_table_path = output_dir / "phase156_uci_superconductivity_audit_table.csv"
    split_manifest_path = output_dir / "phase156_uci_superconductivity_split_manifest.json"
    gate_path = output_dir / "phase156_uci_superconductivity_focused_review_gate.json"
    markdown_path = output_dir / "phase156_uci_superconductivity_focused_review.md"
    manifest_path = output_dir / "phase156_uci_superconductivity_focused_review_manifest.json"
    ...
    _write_json(split_manifest_path, {"phase": 156, "splits": split_manifest})
    _write_json(gate_path, gate)
    with markdown_path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write(build_markdown(gate=gate, split_rows=split_rows, audit_rows=audit_rows))
    manifest = {
        "phase": 156,
        "description": "focused split/shortcut review for Phase 155 UCI superconductivity gate",
        "inputs": {
            "phase155_dir": _display_path(phase155_dir, root),
...
def main() -> None:
    ...
    manifest = build_package(
        root=args.root,
        phase155_dir=args.phase155_dir,
