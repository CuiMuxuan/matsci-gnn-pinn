...
RESIDUAL_BACKBONE_TAG="${RESIDUAL_BACKBONE_TAG:-res_backbone}"
BACKBONE_RESIDUAL_SCALE="${BACKBONE_RESIDUAL_SCALE:-0.5}"

cd "$REPO_ROOT"
mkdir -p logs

run_residual_backbone_holdout() {
  local split_strategy="$1"
  local train_fraction="$2"
  local val_fraction="$3"
  local test_fraction="$4"
  local run_id="ambench_multiline_process_temperature_broad${DATASET_LIMIT}_${DATASET_ORDER}_${split_strategy}_${RESIDUAL_BACKBONE_TAG}_smoke_a100_sxm4_40gb_v1"
...
for split_strategy in $PROFILE_SPLITS; do
  case "$split_strategy" in
    line)
      run_residual_backbone_holdout line 0.6 0.2 0.2
      ;;
    laser_power)
      run_residual_backbone_holdout laser_power 0.34 0.33 0.33
      ;;
    scan_speed)
      run_residual_backbone_holdout scan_speed 0.34 0.33 0.33
      ;;
    spot_size)
      run_residual_backbone_holdout spot_size 0.34 0.33 0.33
      ;;
    process)
      run_residual_backbone_holdout process 0.6 0.2 0.2
      ;;
    *)
