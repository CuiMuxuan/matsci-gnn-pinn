...
Runs no-training feature-profile stress tests on the Phase 108 selected
sequence target to distinguish a genuine modeling gap from a camera-code or
layer-order shortcut.
"""
...
TARGET = "target_cp_camera_pair_delta"
...
def _pair_diagnostics(rows: list[dict[str, str]], target: str = TARGET) -> dict[str, Any]:
    ...
    for layer_rows in two_camera_layers:
        ...
        if abs(sum(values)) <= 1e-6:
            antisymmetric += 1
        if all(
            abs(abs(float(row[target])) - float(row["target_cp_layer_camera_range"])) <= 1e-6
    ...
    return {
        ...
        "antisymmetric_pair_count": antisymmetric,
        "abs_delta_equals_layer_range_pair_count": abs_equals_range,
        "all_two_camera_pairs_antisymmetric": antisymmetric == len(two_camera_layers),
        "all_abs_delta_equals_layer_range": abs_equals_range == len(two_camera_layers),
        "camera_target_mean": {
            camera: sum(values) / len(values) for camera, values in sorted(by_camera.items())
        },
        "camera_positive_fraction": {
            camera: sum(value > 0 for value in values) / len(values)
...
def _profile_rows(
    ...
    *,
    ...
    full_val, full_method = min(full_val_candidates)
    ...
    for profile in PROFILE_FEATURES:
        ...
        selected_val_norm = normalized.get((profile, selected_method, "val"))
        selected_test_norm = normalized.get((profile, selected_method, "test"))
        validation_delta_vs_full = selected_val - full_val
        test_delta_vs_full = selected_test - full_test
        shortcut_like = profile in {"camera_only", "layer_time_camera"} and abs(validation_delta_vs_full) <= min_shortcut_val_rmse_delta
        if profile == "full_phase108":
            status = "reference_full_phase108_profile"
        elif profile == "mean_guard":
            status = "reference_mean_guard"
        elif shortcut_like:
            status = "shortcut_matches_full_validation"
        elif selected_val >= mean_val - min_shortcut_val_rmse_delta:
            status = "no_validation_gain_over_mean_guard"
        else:
            status = "profile_has_independent_validation_signal"
        rows.append(
            {
                "profile": profile,
                ...
                "validation_relative_gain_over_mean": (mean_val - selected_val) / mean_val if mean_val else 0.0,
                "test_relative_gain_over_mean": (mean_test - selected_test) / mean_test if mean_test else 0.0,
                "validation_delta_vs_full": validation_delta_vs_full,
                "test_delta_vs_full": test_delta_vs_full,
                "shortcut_like": shortcut_like,
                "status": status,
            }
        )
    return rows
...
def _build_gate(
    *,
    ...
    camera_only = next(row for row in profile_rows if row["profile"] == "camera_only")
    layer_time_camera = next(row for row in profile_rows if row["profile"] == "layer_time_camera")
    no_camera = next(row for row in profile_rows if row["profile"] == "no_camera")
    full = next(row for row in profile_rows if row["profile"] == "full_phase108")
    shortcut_profiles = [
        row["profile"] for row in (camera_only, layer_time_camera) if row["shortcut_like"]
    ]
    shortcut_detected = bool(
        shortcut_profiles
        and no_camera["status"] == "no_validation_gain_over_mean_guard"
        and pair_diagnostics["all_two_camera_pairs_antisymmetric"]
    )
    if not phase108_ready:
        status = "phase109_focused_review_blocked_by_phase108"
        ...
        next_action = "close Phase 108 camera-pair delta target as diagnostic; do not train"
    else:
        status = "phase109_sequence_target_focused_review_ready_mechanism_design"
        next_action = "design a no-training or low-capacity mechanism gate; keep A100 training closed"
    return {
        "status": status,
        ...
        "camera_only_validation_rmse": camera_only["selected_validation_rmse"],
        "camera_only_test_rmse": camera_only["selected_test_rmse"],
        "layer_time_camera_validation_rmse": layer_time_camera["selected_validation_rmse"],
        "layer_time_camera_test_rmse": layer_time_camera["selected_test_rmse"],
        "no_camera_validation_rmse": no_camera["selected_validation_rmse"],
        "no_camera_test_rmse": no_camera["selected_test_rmse"],
        "min_shortcut_val_rmse_delta": min_shortcut_val_rmse_delta,
        "shortcut_profiles": shortcut_profiles,
        "camera_shortcut_detected": shortcut_detected,
        "pair_diagnostics": pair_diagnostics,
        "phase109_focused_review_allowed": False,
        "phase109_model_mechanism_allowed": False,
...
def _write_markdown(path: Path, gate: dict[str, Any], profile_rows: list[dict[str, Any]]) -> None:
    lines = [
        ...
        f"- Camera shortcut detected: `{gate['camera_shortcut_detected']}`",
...
def build_package(
    *,
    ...
    pair_diagnostics = _pair_diagnostics(source_rows)
    ...
    metrics_path = output_dir / "phase109_nist_ammt_sequence_target_focused_review_metric_table.csv"
    profile_path = output_dir / "phase109_nist_ammt_sequence_target_focused_review_profile_table.csv"
    pair_path = output_dir / "phase109_nist_ammt_sequence_target_pair_diagnostics.json"
    gate_path = output_dir / "phase109_nist_ammt_sequence_target_focused_review_gate.json"
    markdown_path = output_dir / "phase109_nist_ammt_sequence_target_focused_review_summary.md"
    manifest_path = output_dir / "phase109_nist_ammt_sequence_target_focused_review_manifest.json"
    _write_csv(metrics_path, metric_rows, METRIC_FIELDS)
    _write_csv(profile_path, profiles, PROFILE_FIELDS)
    _write_json(pair_path, pair_diagnostics)
    _write_json(gate_path, gate)
    _write_markdown(markdown_path, gate, profiles)
    manifest = {
        "phase": 109,
        "objective": "nist_ammt_sequence_target_focused_review_no_training",
        "inputs": {
        ...
        "outputs": {
            ...
            "pair_diagnostics": _display_path(pair_path, root),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("docs/results/phase109_nist_ammt_sequence_target_focused_review_gate"),
    )
