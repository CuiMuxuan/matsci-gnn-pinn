...
PREDICTION_ANCHOR_WEIGHT="${PREDICTION_ANCHOR_WEIGHT:-0.05}"
PREDICTION_ANCHOR_TAG="${PREDICTION_ANCHOR_TAG:-pred_anchor}"

cd "$REPO_ROOT"
mkdir -p logs

run_prediction_anchor_holdout() {
  local dataset_limit="$1"
  local split_strategy="$2"
  local train_fraction="$3"
  local val_fraction="$4"
  local test_fraction="$5"
  local run_id="ambench_multiline_process_temperature_broad${dataset_limit}_${DATASET_ORDER}_${split_strategy}_${PREDICTION_ANCHOR_TAG}_smoke_a100_sxm4_40gb_v1"

  echo "=== ${run_id} prediction_anchor_weight=${PREDICTION_ANCHOR_WEIGHT} ==="
  RUN_ID="$run_id" \
...
for dataset_limit in $DATASET_LIMITS; do
  for split_strategy in $PROFILE_SPLITS; do
    case "$split_strategy" in
      laser_power)
        run_prediction_anchor_holdout "$dataset_limit" laser_power 0.34 0.33 0.33
        ;;
      spot_size)
        run_prediction_anchor_holdout "$dataset_limit" spot_size 0.34 0.33 0.33
        ;;
      scan_speed)
        run_prediction_anchor_holdout "$dataset_limit" scan_speed 0.34 0.33 0.33
        ;;
      line)
        run_prediction_anchor_holdout "$dataset_limit" line 0.6 0.2 0.2
        ;;
      process)
        run_prediction_anchor_holdout "$dataset_limit" process 0.6 0.2 0.2
        ;;
      *)
