...
run_one() {
  ...
  local hidden="$4"
  local sample_size="$5"
  local run_id="${dataset_id}_macro_pinn_sparse_closure_h${hidden}_l4_lr1e_3_pde_1e_6_residual_${sample_size}_v1"
...
for sample_size in 2048 4096; do
  run_one "$UNIFORM_ID" "$UNIFORM_TABLE" "$UNIFORM_SPLIT" 128 "$sample_size"
  run_one "$ACTIVE_ID" "$ACTIVE_TABLE" "$ACTIVE_SPLIT" 256 "$sample_size"
done
