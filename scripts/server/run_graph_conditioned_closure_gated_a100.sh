...
run_one() {
  ...
  local graph_l1="$2"
  local tag="$3"
  local run_id="${ACTIVE_ID}_macro_pinn_graph_gated_sparse_closure_h256_l4_lr1e_3_clr1e_5_staged1500_random4096_coordinate_rbf_${tag}_v1"
  ...
  "$CONDA_BIN" run -n "$CONDA_ENV" python -m gnnpinn.train.macro_pinn \
    ...
    --closure-graph-embedding-dim 6 \
    --closure-graph-length-scale 0.25 \
...
run_one 0.10 1e-4 gate0_10_gl1e_4
run_one 0.25 1e-4 gate0_25_gl1e_4
