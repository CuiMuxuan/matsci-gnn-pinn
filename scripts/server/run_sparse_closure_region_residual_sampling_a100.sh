...
run_one() {
  local mode="$1"
  local sample_size="$2"
  local run_id="${ACTIVE_ID}_macro_pinn_sparse_closure_h256_l4_lr1e_3_pde_1e_6_residual_${mode}_${sample_size}_v1"
...
for mode in hot gradient hot_gradient; do
  for sample_size in 2048 4096; do
    run_one "$mode" "$sample_size"
  done
done
