...
PROCESS_FEATURE_TAG="${PROCESS_FEATURE_TAG:-phys_proc}"

cd "$REPO_ROOT"
mkdir -p logs

run_derived_process_holdout() {
...
for split_strategy in $PROFILE_SPLITS; do
  case "$split_strategy" in
    laser_power)
      run_derived_process_holdout laser_power 0.34 0.33 0.33
      ;;
    spot_size)
      run_derived_process_holdout spot_size 0.34 0.33 0.33
      ;;
    scan_speed)
      run_derived_process_holdout scan_speed 0.34 0.33 0.33
      ;;
    line)
      run_derived_process_holdout line 0.6 0.2 0.2
      ;;
    process)
      run_derived_process_holdout process 0.6 0.2 0.2
      ;;
    *)
