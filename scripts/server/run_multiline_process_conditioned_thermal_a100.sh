...
run_macro_pinn() {
  ...
  if [[ -n "$DATA_LOSS_GROUP_BALANCE_COLUMN" ]]; then
    data_loss_weighting_args+=(
      --data-loss-group-balance-column "$DATA_LOSS_GROUP_BALANCE_COLUMN"
      --data-loss-group-balance-strength "$DATA_LOSS_GROUP_BALANCE_STRENGTH"
    )
  fi
