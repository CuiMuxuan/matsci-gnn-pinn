...
run_one() {
  ...
  local run_id="${dataset_id}_macro_pinn_sparse_closure_h${hidden}_l4_lr1e_3_pde_${weight_tag}_low_v1"
...
for weight in 1e-5 1e-6; do
  run_one "$UNIFORM_ID" "$UNIFORM_TABLE" "$UNIFORM_SPLIT" 128 "$weight"
  run_one "$ACTIVE_ID" "$ACTIVE_TABLE" "$ACTIVE_SPLIT" 256 "$weight"
done
