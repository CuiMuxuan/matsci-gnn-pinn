#!/usr/bin/env python3
"""Build a Phase 141 baseline-first gate for Matbench experimental is-metal.

This phase opens a small public external source after the Phase 140 large-source
download blocker. It downloads Matbench v0.1 ``matbench_expt_is_metal`` if
needed, parses formulas through the Phase 123 composition descriptor route, and
reviews the binary ``is_metal`` target with strong classification baselines and
...
SOURCE_URL = "https://ml.materialsproject.org/projects/matbench_expt_is_metal.json.gz"
DEFAULT_RAW_PATH = Path("data/raw/external/matbench_expt_is_metal/matbench_expt_is_metal.json.gz")
DEFAULT_OUTPUT_DIR = Path("docs/results/phase141_matbench_expt_is_metal_baseline_gate")
EXPECTED_MIN_BYTES = 20_000
EXPECTED_HEAD_BYTES = 34_623
...
def ensure_source_file(path: Path, *, source_url: str, force_download: bool = False) -> dict[str, Any]:
    ...
    if force_download or not path.exists() or path.stat().st_size < EXPECTED_MIN_BYTES:
        request = urllib.request.Request(
            source_url,
            headers={"User-Agent": "Mozilla/5.0 Codex Phase141 matbench expt is-metal gate"},
        )
...
def load_source_table(path: Path) -> pd.DataFrame:
    ...
    if payload.get("columns") != ["composition", TARGET_COLUMN]:
        raise ValueError(f"Unexpected matbench_expt_is_metal columns: {payload.get('columns')}")
...
def build_field_table(df: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    for index, row in df.reset_index(drop=True).iterrows():
        ...
        rows.append(
            {
                "phase141_row_id": f"MBEXPTMETAL-{len(rows):05d}",
...
def _fit_predict_proba(method: str, x_train: np.ndarray, y_train: np.ndarray, x_all: np.ndarray) -> np.ndarray:
    ...
    if method == "knn":
        ...
        model = ExtraTreesClassifier(n_estimators=128, random_state=141, n_jobs=1, class_weight="balanced")
    elif method == "hist_gradient_boosting":
        from sklearn.ensemble import HistGradientBoostingClassifier

        model = HistGradientBoostingClassifier(max_iter=160, random_state=141, early_stopping=False)
    else:
...
def evaluate_target(df: pd.DataFrame, split_manifest: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    val_gain = float(best_val["balanced_accuracy"] - majority_val["balanced_accuracy"])
    test_gain = float(best_test["balanced_accuracy"] - majority_test["balanced_accuracy"])
    shortcut_blocks = bool(
        best_negative_val["balanced_accuracy"] >= best_val["balanced_accuracy"] - SHORTCUT_DOMINANCE_MARGIN
    )
    candidate = bool(
        val_gain >= MIN_BALANCED_ACCURACY_GAIN
        and test_gain > 0.0
        and best_test["balanced_accuracy"] >= majority_test["balanced_accuracy"]
        and not shortcut_blocks
    )
    if shortcut_blocks:
        status = "blocked_shortcut_dominance"
        reason = "best shortcut/control profile matches or exceeds admissible validation balanced accuracy"
    elif val_gain < MIN_BALANCED_ACCURACY_GAIN:
        status = "closed_no_validation_gain"
        reason = "best admissible profile does not beat majority by the validation gain guard"
    elif test_gain <= 0.0 or best_test["balanced_accuracy"] < majority_test["balanced_accuracy"]:
        status = "closed_validation_test_reversal"
        reason = "validation-selected profile does not preserve a test balanced-accuracy gain"
    else:
        status = "ready_focused_review"
        reason = "composition profile beats majority and shortcut controls"
    review = {
        "target": TARGET_COLUMN,
        "row_count": int(len(df)),
        "train_rows": len(splits["train"]),
...
def build_schema_rows(df: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    for column in df.columns:
        ...
        if column == TARGET_COLUMN:
            role = "target"
        elif column in {"phase141_row_id", "composition"}:
            role = "identifier"
...
def build_gate(
    *,
    ...
    focused_allowed = bool(review.get("phase141_candidate"))
    if focused_allowed:
        ...
        status = "phase141_matbench_expt_is_metal_incomplete_insufficient_split_rows"
        next_action = "repair split or choose another public target"
    else:
        status = "phase141_matbench_expt_is_metal_closed_no_stable_guarded_gap"
        next_action = "close as external-data diagnostic or choose another public target"
    return {
        "status": status,
        "source_name": "Matbench v0.1 matbench_expt_is_metal",
        ...
        "phase141_focused_review_allowed": focused_allowed,
        "phase141_model_mechanism_allowed": False,
        "phase141_model_training_allowed": False,
...
def build_data_card(source_info: dict[str, Any], gate: dict[str, Any], parse_audit: dict[str, Any]) -> dict[str, Any]:
    return {
        "dataset": "Matbench v0.1 matbench_expt_is_metal",
        "source_url": source_info["source_url"],
        "source_sha256": source_info["sha256"],
        "source_byte_size": source_info["byte_size"],
        "license_note": "Public Matbench benchmark dataset; cite Matbench/matminer in manuscripts.",
        "target": "is_metal binary experimental metallicity label",
...
def build_markdown(gate: dict[str, Any], review: dict[str, Any]) -> str:
    lines = [
        ...
        f"- Focused review allowed: `{gate['phase141_focused_review_allowed']}`",
        f"- Model training allowed: `{gate['phase141_model_training_allowed']}`",
...
def build_package(
    *,
    ...
    prefix = "phase141_matbench_expt_is_metal"
    ...
    manifest = {
        "phase": 141,
        "objective": "matbench_expt_is_metal_classification_baseline_first_gate_no_training",
        "source": source_info,
        ...
        "counts": {
            ...
            "candidate_targets": 1 if gate["phase141_focused_review_allowed"] else 0,
        },
