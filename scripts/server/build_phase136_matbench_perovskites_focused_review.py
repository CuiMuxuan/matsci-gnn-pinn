#!/usr/bin/env python3
"""Build Phase 136 focused review for the Matbench perovskites gate.

This phase consumes only small Phase 135 artifacts. It checks whether the
Phase 135 perovskites formation-energy gap is stable under alternate
grouped/binned splits and whether shortcut profiles, nearest-neighbor
composition/lattice identity, or target-distribution imbalance explain the
signal. It does not train a neural model or open A100/A800 training.
"""
...
DEFAULT_PHASE135_DIR = Path("docs/results/phase135_matbench_perovskites_baseline_gate")
DEFAULT_OUTPUT_DIR = Path("docs/results/phase136_matbench_perovskites_focused_review")
MIN_SPLIT_ROWS = 100
...
phase135 = _load_module("build_phase135_matbench_perovskites_baseline_gate.py", "phase135_helpers")
phase127 = _load_module("build_phase127_matbench_phonons_focused_review.py", "phase127_helpers")
phase130 = phase135.phase130
...
SPLIT_PLAN = (
    ...
    ("chemistry_family_hash_0", "group:chemistry_family_key", "phase136_family_0"),
    ("chemistry_family_hash_1", "group:chemistry_family_key", "phase136_family_1"),
    ("chemistry_family_hash_2", "group:chemistry_family_key", "phase136_family_2"),
    ("dominant_element_hash", "group:dominant_element", "phase136_dominant"),
    ("lattice_volume_bins", "bins:lattice_volume", "phase136_lattice_volume"),
    ("volume_per_site_bins", "bins:volume_per_site", "phase136_volume_per_site"),
    ("element_count_bins", "bins:element_count", "phase136_element_count"),
    ("density_anisotropy_bins", "bins:density_z_proxy,abc_anisotropy", "phase136_density_aniso"),
)
...
def _registered_split(split_manifest: dict[str, Any]) -> dict[str, Any]:
    return {
        "split_strategy": split_manifest.get("split_strategy", "phase135_registered_split"),
        "split_salt": "phase135",
        "split_viable": True,
        "reason": "ok",
...
def build_split_reviews(df: pd.DataFrame, split_manifest: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    for split_id, split_spec, salt in SPLIT_PLAN:
        if split_spec == "phase135_manifest":
...
def build_audit_rows(*, phase135_gate: dict[str, Any], split_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    phase135_ready = phase135_gate.get("status") == "phase135_matbench_perovskites_ready_focused_review"
    rows.append(
        _audit_row(
            audit="phase135_gate_status",
            status="pass" if phase135_ready else "block",
            severity="blocking" if not phase135_ready else "info",
            value=phase135_gate.get("status"),
            threshold="phase135_matbench_perovskites_ready_focused_review",
            reason="focused review requires a Phase 135 focused-review gate",
        )
    )
    original = next((row for row in split_rows if row.get("split_id") == "phase135_registered_split"), None)
    if original and original.get("split_viable"):
        rows.append(
            _audit_row(
                ...
                reason="Phase 135 selected perovskites target must preserve validation and test gain",
            )
        )
        rows.append(
            _audit_row(
        ...
        rows.append(
            _audit_row(
                ...
                reason="Phase 135 registered split must be reviewable",
            )
        )
    ...
    rows.append(
        _audit_row(
            ...
            reason="perovskites target gain must survive deterministic chemistry, dominant-element, and lattice split perturbations",
        )
    )
    ...
    rows.append(
        _audit_row(
            ...
            threshold=0,
            reason="no viable split may have severe train/validation/test perovskites target mean, median, or q90 imbalance",
        )
    )
    return rows


def build_gate(*, phase135_gate: dict[str, Any], split_rows: list[dict[str, Any]], audit_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    original = next((row for row in split_rows if row.get("split_id") == "phase135_registered_split"), {})
    if phase135_gate.get("status") != "phase135_matbench_perovskites_ready_focused_review":
        status = "phase136_matbench_perovskites_review_blocked_by_phase135"
        mechanism_allowed = False
        next_action = "complete or close Phase 135 before focused review"
    elif blockers:
        status = "phase136_matbench_perovskites_focused_review_closed_split_sensitivity_or_shortcut"
        mechanism_allowed = False
        next_action = "close the Phase 135 perovskites target as diagnostic; do not train"
    else:
        status = "phase136_matbench_perovskites_focused_review_ready_low_capacity_mechanism_gate"
        mechanism_allowed = True
        next_action = "design a separate no-training low-capacity perovskites mechanism gate; keep model training closed"
    return {
        "status": status,
        "phase135_status": phase135_gate.get("status"),
        "selected_target": phase135_gate.get("selected_target", "e_form"),
        "phase135_selected_profile": phase135_gate.get("selected_profile"),
        "phase135_selected_method": phase135_gate.get("selected_method"),
        ...
        "phase136_model_mechanism_allowed": mechanism_allowed,
        "phase136_low_capacity_mechanism_design_allowed": mechanism_allowed,
        "phase136_model_training_allowed": False,
...
def build_markdown(gate: dict[str, Any], audit_rows: list[dict[str, Any]], split_rows: list[dict[str, Any]]) -> str:
    ...
    lines = [
        ...
        f"- Low-capacity mechanism design allowed: `{gate['phase136_low_capacity_mechanism_design_allowed']}`",
        f"- Model training allowed: `{gate['phase136_model_training_allowed']}`",
        "",
        "## Blocking Audits",
        "",
...
def build_package(*, root: Path, phase135_dir: Path, output_dir: Path) -> dict[str, Any]:
    _configure_phase127_helpers()
    field_path = phase135_dir / "phase135_matbench_perovskites_field_table.csv"
    split_path = phase135_dir / "phase135_matbench_perovskites_split_manifest.json"
    gate_path = phase135_dir / "phase135_matbench_perovskites_gate.json"
    df = pd.read_csv(field_path)
    split_manifest = _read_json(split_path)
    phase135_gate = _read_json(gate_path)
    target = str(phase135_gate.get("selected_target") or "e_form")
    ...
    audit_rows = build_audit_rows(phase135_gate=phase135_gate, split_rows=split_rows)
    gate = build_gate(phase135_gate=phase135_gate, split_rows=split_rows, audit_rows=audit_rows)

    output_dir.mkdir(parents=True, exist_ok=True)
    profile_path = output_dir / "phase136_matbench_perovskites_focused_profile_table.csv"
    split_review_path = output_dir / "phase136_matbench_perovskites_split_sensitivity_table.csv"
    audit_path = output_dir / "phase136_matbench_perovskites_shortcut_audit_table.csv"
    split_manifest_path = output_dir / "phase136_matbench_perovskites_split_review_manifest.json"
    gate_out = output_dir / "phase136_matbench_perovskites_focused_review_gate.json"
    markdown_path = output_dir / "phase136_matbench_perovskites_focused_review.md"
    manifest_path = output_dir / "phase136_matbench_perovskites_focused_review_manifest.json"
    ...
    manifest = {
        "phase": 136,
        "objective": "matbench_perovskites_focused_split_shortcut_review_no_training",
        "inputs": {
            "phase135_dir": phase127._display_path(phase135_dir, root),
            "field_table": phase127._display_path(field_path, root),
            "split_manifest": phase127._display_path(split_path, root),
            "phase135_gate": phase127._display_path(gate_path, root),
        },
        "outputs": {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--phase135-dir", type=Path, default=DEFAULT_PHASE135_DIR)
...
def main() -> None:
    ...
    phase135_dir = args.phase135_dir if args.phase135_dir.is_absolute() else root / args.phase135_dir
    output_dir = args.output_dir if args.output_dir.is_absolute() else root / args.output_dir
    manifest = build_package(root=root, phase135_dir=phase135_dir, output_dir=output_dir)
