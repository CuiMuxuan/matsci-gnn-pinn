...
run_broad_profile_holdout() {
  local split_strategy="$1"
  local train_fraction="$2"
  local val_fraction="$3"
  local test_fraction="$4"
  local run_id="ambench_multiline_process_temperature_broad${DATASET_LIMIT}_${DATASET_ORDER}_${split_strategy}_process_axis_profile_smoke_a100_sxm4_40gb_v1"
...
for split_strategy in $PROFILE_SPLITS; do
  case "$split_strategy" in
    line)
      run_broad_profile_holdout line 0.6 0.2 0.2
      ;;
    laser_power)
      run_broad_profile_holdout laser_power 0.34 0.33 0.33
      ;;
    scan_speed)
      run_broad_profile_holdout scan_speed 0.34 0.33 0.33
      ;;
    spot_size)
      run_broad_profile_holdout spot_size 0.34 0.33 0.33
      ;;
    process)
      run_broad_profile_holdout process 0.6 0.2 0.2
      ;;
    *)
