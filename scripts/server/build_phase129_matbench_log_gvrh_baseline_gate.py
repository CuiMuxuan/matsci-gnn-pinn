#!/usr/bin/env python3
"""Build a Phase 129 baseline-first gate for Matbench log GVRH.
...
``matbench_log_gvrh`` if needed, parses structure JSON dictionaries into
composition and lattice descriptors, and reviews the shear-modulus target
``log10(G_VRH)`` with strong tabular baselines and shortcut controls. It does
not train a neural model or open A100 training.
"""
...
SOURCE_URL = "https://ml.materialsproject.org/projects/matbench_log_gvrh.json.gz"
DEFAULT_RAW_PATH = Path("data/raw/external/matbench_log_gvrh/matbench_log_gvrh.json.gz")
DEFAULT_OUTPUT_DIR = Path("docs/results/phase129_matbench_log_gvrh_baseline_gate")
EXPECTED_MIN_BYTES = 1_000_000
EXPECTED_HEAD_BYTES = 4_166_214
...
def ensure_source_file(path: Path, *, source_url: str, force_download: bool = False) -> dict[str, Any]:
    ...
    if force_download or not path.exists() or path.stat().st_size < EXPECTED_MIN_BYTES:
        request = urllib.request.Request(
            source_url,
            headers={"User-Agent": "Mozilla/5.0 Codex Phase129 matbench log_gvrh gate"},
        )
...
def load_matbench_payload(path: Path) -> tuple[list[str], list[list[Any]]]:
    ...
    if columns != ["structure", "log10(G_VRH)"]:
        raise ValueError(f"Unexpected matbench_log_gvrh columns: {columns}")
...
def build_field_table(rows: list[list[Any]]) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    for index, row in enumerate(rows):
        ...
        try:
            ...
            skipped_rows.append({"row_index": index, "reason": str(exc)})
            continue
        ...
        record = {
            "phase129_row_id": f"P129-{index:05d}",
            "composition": composition,
            "log10_g_vrh": float(target),
    ...
    return pd.DataFrame(out_rows, columns=list(FIELD_FIELDS)), {
        "raw_rows": len(rows),
        "parsed_rows": len(out_rows),
        "skipped_rows": len(skipped_rows),
        "skipped_row_examples": skipped_rows[:10],
    }


def group_split(df: pd.DataFrame, *, group_column: str = "chemistry_family_key", salt: str = "phase129") -> dict[str, Any]:
...
def build_schema_rows(df: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    for column in df.columns:
        ...
        role = "feature"
        if column == "log10_g_vrh":
            role = "target"
        elif column in {"phase129_row_id", "composition"}:
            role = "identifier"
...
def build_gate(*, source_info: dict[str, Any], split_manifest: dict[str, Any], review: dict[str, Any]) -> dict[str, Any]:
    status = (
        ...
        if review.get("phase129_candidate")
        else "phase129_matbench_log_gvrh_closed_no_stable_guarded_gap"
    )
    return {
        "status": status,
        ...
        "phase129_focused_review_allowed": bool(review.get("phase129_candidate")),
        "phase129_model_mechanism_allowed": False,
        "phase129_model_training_allowed": False,
...
def build_markdown(gate: dict[str, Any], review: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            f"- Focused review allowed: `{gate['phase129_focused_review_allowed']}`",
            f"- Model training allowed: `{gate['phase129_model_training_allowed']}`",
...
def build_package(
    *,
    ...
    field_path = output_dir / "phase129_matbench_log_gvrh_field_table.csv"
    split_path = output_dir / "phase129_matbench_log_gvrh_split_manifest.json"
    metric_path = output_dir / "phase129_matbench_log_gvrh_metric_table.csv"
    review_path = output_dir / "phase129_matbench_log_gvrh_target_review_table.csv"
    schema_path = output_dir / "phase129_matbench_log_gvrh_schema_table.csv"
    data_card_path = output_dir / "phase129_matbench_log_gvrh_data_card.json"
    gate_path = output_dir / "phase129_matbench_log_gvrh_gate.json"
    markdown_path = output_dir / "phase129_matbench_log_gvrh.md"
    manifest_path = output_dir / "phase129_matbench_log_gvrh_manifest.json"
    ...
    manifest = {
        "phase": 129,
        "objective": "matbench_log_gvrh_baseline_first_gate_no_training",
        "source": source_info,
