#!/usr/bin/env python3
"""Build Phase 124 focused review for the Matbench experimental gap gate.

This phase consumes only small Phase 123 artifacts. It checks whether the
Phase 123 chemistry-property gap is stable under alternate grouped/binned
splits and whether shortcut profiles, nearest-neighbor composition identity, or
zero-gap split imbalance explain the signal. It does not train a neural model
or open A100 training.
"""
...
DEFAULT_OUTPUT_DIR = Path("docs/results/phase124_matbench_expt_gap_focused_review")
MIN_SPLIT_ROWS = 200
MIN_RELATIVE_VAL_GAIN = 0.05
MIN_STABLE_SPLIT_PASS_RATE = 0.80
NEGATIVE_DOMINANCE_TOLERANCE = 1.02
MAX_ORIGINAL_ZERO_GAP_RATE_DELTA = 0.15
MAX_NEAR_DUPLICATE_FRACTION = 0.50
NEAR_DUPLICATE_L1_DISTANCE = 0.02
...
PROFILE_COLUMNS: dict[str, dict[str, Any]] = {
    "common_element_fractions": {
        **phase123.PROFILE_COLUMNS["common_element_fractions"],
        "role": "admissible",
    },
    "all_element_fractions": {
        **phase123.PROFILE_COLUMNS["all_element_fractions"],
        "role": "admissible",
    },
    "chemistry_descriptors": {
        **phase123.PROFILE_COLUMNS["chemistry_descriptors"],
        "role": "admissible",
    },
    "common_plus_descriptors": {
        **phase123.PROFILE_COLUMNS["common_plus_descriptors"],
        "role": "admissible",
    },
    "composition_hash_shortcut": {
        **phase123.PROFILE_COLUMNS["composition_hash_shortcut"],
...
PROFILE_METHODS = {
    ...
    # High-cardinality chemistry-family one-hot ExtraTrees is a runtime-only
    # stressor here; kNN is the intended exact family shortcut control.
...
SPLIT_PLAN = (
    ...
    ("chemistry_family_hash_0", "group:chemistry_family_key", "phase124_family_0"),
    ("chemistry_family_hash_1", "group:chemistry_family_key", "phase124_family_1"),
    ("chemistry_family_hash_2", "group:chemistry_family_key", "phase124_family_2"),
    ("chemistry_family_hash_3", "group:chemistry_family_key", "phase124_family_3"),
    ("chemistry_family_hash_4", "group:chemistry_family_key", "phase124_family_4"),
    ("dominant_element_hash", "group:dominant_element", "phase124_dominant"),
    ("anion_electronegativity_bins", "bins:anion_fraction,mean_electronegativity,electronegativity_range", "phase124_anion_en"),
    ("family_fraction_bins", "bins:chalcogen_fraction,halogen_fraction,transition_metal_fraction", "phase124_family_frac"),
)
...
def _phase123_split(split_manifest: dict[str, Any]) -> dict[str, Any]:
    return {
        "split_strategy": split_manifest.get("split_strategy", "phase123_registered_split"),
        "split_salt": "phase123",
        "split_viable": True,
        "reason": "ok",
...
def build_split_reviews(df: pd.DataFrame, split_manifest: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    for split_id, split_spec, salt in SPLIT_PLAN:
        if split_spec == "phase123_manifest":
            reviews[split_id] = _phase123_split(split_manifest)
...
def _nearest_neighbor_control(
    ...
    *,
    ...
    feature_columns = [f"frac_{element}" for element in phase123.ELEMENTS if f"frac_{element}" in df.columns]
    x_all = df[feature_columns].apply(pd.to_numeric, errors="coerce").fillna(0.0).to_numpy(dtype=float)
    ...
    val_metrics = phase123._metrics(y[splits["val"]], pred[splits["val"]], train_std)
    test_metrics = phase123._metrics(y[splits["test"]], pred[splits["test"]], train_std)
    ...
    return {
        ...
        "val_near_duplicate_fraction": float(np.mean(val_dist <= NEAR_DUPLICATE_L1_DISTANCE)) if len(val_dist) else 0.0,
        "test_near_duplicate_fraction": float(np.mean(test_dist <= NEAR_DUPLICATE_L1_DISTANCE)) if len(test_dist) else 0.0,
    }
...
def evaluate_split(
    ...
    *,
    ...
    mean_val = phase123._metrics(y[splits["val"]], mean_pred[splits["val"]], train_std)
    mean_test = phase123._metrics(y[splits["test"]], mean_pred[splits["test"]], train_std)
    ...
    for profile_name, profile in PROFILE_COLUMNS.items():
        ...
        for method in PROFILE_METHODS.get(profile_name, MODEL_METHODS):
            ...
            split_metrics = {
                split: phase123._metrics(y[splits[split]], pred[splits[split]], train_std)
                for split in ("train", "val", "test")
            }
    ...
    zero_rates = {
        split: float(np.mean(y[splits[split]] <= 0.0)) if splits[split] else 0.0
        for split in ("train", "val", "test")
    }
    zero_gap_delta = max(
        abs(zero_rates["val"] - zero_rates["train"]),
        abs(zero_rates["test"] - zero_rates["train"]),
    )
    ...
    summary = {
        **base,
        "split_viable": True,
        "split_reason": "ok",
        "train_zero_gap_rate": zero_rates["train"],
        "val_zero_gap_rate": zero_rates["val"],
        "test_zero_gap_rate": zero_rates["test"],
        "zero_gap_rate_delta": zero_gap_delta,
        "mean_val_rmse": mean_val["rmse"],
        "mean_test_rmse": mean_test["rmse"],
...
def build_audit_rows(*, phase123_gate: dict[str, Any], split_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    ...
    rows.append(
        {
            ...
            "reason": "focused review requires a Phase 123 focused-review gate",
        }
    )
    original = next((row for row in split_rows if row.get("split_id") == "phase123_registered_split"), None)
        ...
        rows.append(
            {
                ...
                "reason": "Phase 123 selected chemistry target must preserve validation and test gain",
            }
        )
        rows.append(
            {
                ...
                "reason": "composition identity or chemistry-family shortcuts must not dominate the selected profile",
            }
        )
        rows.append(
            {
                ...
                "reason": "nearest-neighbor composition identity control must not dominate the selected profile",
            }
        )
        rows.append(
            {
                "audit": "original_split_zero_gap_balance",
                "status": "block" if float(original.get("zero_gap_rate_delta") or 0.0) > MAX_ORIGINAL_ZERO_GAP_RATE_DELTA else "pass",
                "severity": "blocking" if float(original.get("zero_gap_rate_delta") or 0.0) > MAX_ORIGINAL_ZERO_GAP_RATE_DELTA else "info",
                "value": original.get("zero_gap_rate_delta"),
                "threshold": MAX_ORIGINAL_ZERO_GAP_RATE_DELTA,
                "reason": "registered split must not be explained by severe zero-gap class-balance mismatch",
            }
        )
        rows.append(
            {
                ...
                "reason": "registered split must not contain too many near-duplicate compositions relative to train",
            }
        )
    else:
        rows.append(
            {
                ...
                "reason": "Phase 123 registered split must be reviewable",
            }
        )
    ...
    rows.append(
        {
            ...
            "reason": "chemistry-profile gain must survive deterministic chemistry-family and binned split perturbations",
        }
    )
    ...
    rows.append(
        {
            ...
            "threshold": 0,
            "reason": "no viable split may be dominated by chemistry identity or family shortcut profiles",
        }
    )
...
def build_gate(*, phase123_gate: dict[str, Any], split_rows: list[dict[str, Any]], audit_rows: list[dict[str, Any]]) -> dict[str, Any]:
    blockers = [row for row in audit_rows if row["status"] == "block"]
    viable = [row for row in split_rows if _is_true(row.get("split_viable"))]
    passed = [row for row in viable if _is_true(row.get("split_pass"))]
    shortcut_dominant = [row for row in viable if _is_true(row.get("negative_control_dominates"))]
    nearest_dominant = [row for row in viable if _is_true(row.get("nearest_neighbor_dominates"))]
    original = next((row for row in split_rows if row.get("split_id") == "phase123_registered_split"), {})
    if phase123_gate.get("status") != "phase123_matbench_expt_gap_gap_ready_focused_review":
        status = "phase124_matbench_expt_gap_review_blocked_by_phase123"
        mechanism_allowed = False
        next_action = "complete or close Phase 123 before focused review"
    elif blockers:
        status = "phase124_matbench_expt_gap_focused_review_closed_split_sensitivity_or_shortcut"
        mechanism_allowed = False
        next_action = "close the Phase 123 target as diagnostic; do not train"
    else:
    ...
    return {
        ...
        "selected_target": phase123_gate.get("selected_target", "gap_expt_ev"),
        ...
        "original_zero_gap_rate_delta": original.get("zero_gap_rate_delta"),
        "phase124_model_mechanism_allowed": mechanism_allowed,
        "phase124_low_capacity_mechanism_design_allowed": mechanism_allowed,
...
def build_markdown(gate: dict[str, Any], audit_rows: list[dict[str, Any]], split_rows: list[dict[str, Any]]) -> str:
    ...
    lines = [
        ...
        f"- Low-capacity mechanism design allowed: `{gate['phase124_low_capacity_mechanism_design_allowed']}`",
        f"- Model training allowed: `{gate['phase124_model_training_allowed']}`",
        "",
        "## Blocking Audits",
        "",
        ...
        _markdown_table(
            ...
            [
                ...
                ("Zero delta", "zero_gap_rate_delta"),
            ],
        ),
    ]
    return "\n".join(lines) + "\n"
...
def build_package(*, root: Path, phase123_dir: Path, output_dir: Path) -> dict[str, Any]:
    ...
    gate_path = phase123_dir / "phase123_matbench_expt_gap_gate.json"
    df = pd.read_csv(field_path)
    split_manifest = _read_json(split_path)
    phase123_gate = _read_json(gate_path)
    target = str(phase123_gate.get("selected_target") or "gap_expt_ev")
    ...
    audit_rows = build_audit_rows(phase123_gate=phase123_gate, split_rows=split_rows)
    gate = build_gate(phase123_gate=phase123_gate, split_rows=split_rows, audit_rows=audit_rows)

    output_dir.mkdir(parents=True, exist_ok=True)
    profile_path = output_dir / "phase124_matbench_expt_gap_focused_profile_table.csv"
    split_review_path = output_dir / "phase124_matbench_expt_gap_split_sensitivity_table.csv"
    audit_path = output_dir / "phase124_matbench_expt_gap_shortcut_audit_table.csv"
    split_manifest_path = output_dir / "phase124_matbench_expt_gap_split_review_manifest.json"
    gate_out = output_dir / "phase124_matbench_expt_gap_focused_review_gate.json"
    markdown_path = output_dir / "phase124_matbench_expt_gap_focused_review.md"
    manifest_path = output_dir / "phase124_matbench_expt_gap_focused_review_manifest.json"
    ...
    manifest = {
        "phase": 124,
        "objective": "matbench_expt_gap_focused_split_shortcut_review_no_training",
        "inputs": {
            ...
            "phase123_gate": _display_path(gate_path, root),
        },
        "outputs": {
...
def main() -> None:
    ...
    manifest = build_package(root=root, phase123_dir=phase123_dir, output_dir=output_dir)
