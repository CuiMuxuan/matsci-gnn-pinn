#!/usr/bin/env python3
"""Build a Phase 135 baseline-first gate for Matbench perovskites.

This phase opens a fresh small public external data-source intake after the
Phase 129-134 elastic/2D branches closed as diagnostics. It downloads
Matbench v0.1 ``matbench_perovskites`` if needed, parses structure JSON
dictionaries through the Phase 130 structure descriptor path, and reviews
``e_form`` with strong tabular baselines and shortcut controls. It does not
...
SOURCE_URL = "https://ml.materialsproject.org/projects/matbench_perovskites.json.gz"
DEFAULT_RAW_PATH = Path("data/raw/external/matbench_perovskites/matbench_perovskites.json.gz")
DEFAULT_OUTPUT_DIR = Path("docs/results/phase135_matbench_perovskites_baseline_gate")
EXPECTED_MIN_BYTES = 1_000_000
EXPECTED_HEAD_BYTES = 4_193_112
...
REVIEW_FIELDS = tuple(
    "phase135_candidate" if field == "phase130_candidate" else field
...
def ensure_source_file(path: Path, *, source_url: str, force_download: bool = False) -> dict[str, Any]:
    ...
    if force_download or not path.exists() or path.stat().st_size < EXPECTED_MIN_BYTES:
        request = urllib.request.Request(
            source_url,
            headers={"User-Agent": "Mozilla/5.0 Codex Phase135 matbench perovskites gate"},
        )
...
def load_matbench_payload(path: Path) -> tuple[list[str], list[list[Any]]]:
    ...
    if columns != ["structure", TARGET_COLUMN]:
        raise ValueError(f"Unexpected matbench_perovskites columns: {columns}")
...
def build_field_table(rows: list[list[Any]]) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    field_df = field_df.rename(
        columns={"phase130_row_id": "phase135_row_id", "log10_k_vrh": TARGET_COLUMN}
    )
    field_df["phase135_row_id"] = [f"P135-{index:05d}" for index in range(len(field_df))]
    return field_df.loc[:, list(FIELD_FIELDS)], parse_audit


def group_split(df: pd.DataFrame, *, group_column: str = "chemistry_family_key", salt: str = "phase135") -> dict[str, Any]:
    return phase130.group_split(df, group_column=group_column, salt=salt)


def _as_phase130_frame(df: pd.DataFrame) -> pd.DataFrame:
    return df.rename(columns={"phase135_row_id": "phase130_row_id", TARGET_COLUMN: "log10_k_vrh"})
...
def evaluate_target(df: pd.DataFrame, split_manifest: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    review["phase135_candidate"] = bool(review.pop("phase130_candidate", False))
...
def build_gate(*, source_info: dict[str, Any], split_manifest: dict[str, Any], review: dict[str, Any]) -> dict[str, Any]:
    status = (
        "phase135_matbench_perovskites_ready_focused_review"
        if review.get("phase135_candidate")
    ...
    return {
        ...
        "phase135_focused_review_allowed": bool(review.get("phase135_candidate")),
...
def build_markdown(gate: dict[str, Any], review: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# Phase 135 Matbench Perovskites Baseline Gate",
            "",
            ...
            f"- Focused review allowed: `{gate['phase135_focused_review_allowed']}`",
            f"- Model training allowed: `{gate['phase135_model_training_allowed']}`",
...
def build_package(
    *,
    ...
    field_path = output_dir / "phase135_matbench_perovskites_field_table.csv"
    split_path = output_dir / "phase135_matbench_perovskites_split_manifest.json"
    metric_path = output_dir / "phase135_matbench_perovskites_metric_table.csv"
    review_path = output_dir / "phase135_matbench_perovskites_target_review_table.csv"
    schema_path = output_dir / "phase135_matbench_perovskites_schema_table.csv"
    data_card_path = output_dir / "phase135_matbench_perovskites_data_card.json"
    gate_path = output_dir / "phase135_matbench_perovskites_gate.json"
    markdown_path = output_dir / "phase135_matbench_perovskites.md"
    manifest_path = output_dir / "phase135_matbench_perovskites_manifest.json"
    ...
    _write_json(
        data_card_path,
        {
            ...
            "phase135_no_training": True,
            "a800_reproduction_required": True,
        },
    )
    ...
    manifest = {
        "phase": 135,
        "objective": "matbench_perovskites_baseline_first_gate_no_training",
        "source": source_info,
