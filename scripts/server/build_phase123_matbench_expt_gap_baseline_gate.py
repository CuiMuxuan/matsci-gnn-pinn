...
def _fit_predict(method: str, x_train: np.ndarray, y_train: np.ndarray, x_all: np.ndarray) -> np.ndarray:
    if method == "knn":
        ...
        model = ExtraTreesRegressor(n_estimators=240, random_state=123, n_jobs=-1)
    elif method == "hist_gradient_boosting":
        from sklearn.ensemble import HistGradientBoostingRegressor

        model = HistGradientBoostingRegressor(max_iter=220, random_state=123, early_stopping=False)
    else:
