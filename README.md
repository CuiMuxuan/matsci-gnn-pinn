...
- Phase 45 baseline-guarded process expert Gate 1 已关闭为负向诊断。prediction stack probe 只用 train/validation 选择权重，但 broad12 stack `143.594749 / 286.768169 / 229.955382` 弱于 mean 和 `broad_process_v1`；broad21 stack `160.353180 / 321.588710 / 255.299857` 弱于 mean，且 hot/gradient 弱于 `broad_process_v1`。因此不实现 trainable guarded expert，不做 seed expansion。
- Phase 46 Bayesian inverse-closure PINN 已完成本地 feasibility validation。synthetic heat-source probe 能恢复低维 source 参数，但 uncertainty-guided sampling 没有同时守住 global/hot/gradient/calibration；本地 AM-Bench `Line_0_1` sparse proxy 改善 global RMSE 但显著伤害 hot q90 且 coverage 不足。因此 Phase 46 关闭为负向诊断，不扩展到 A100 broad12/broad21。
- Phase 47 lightweight physics-guided closure attention probe 已关闭为负向诊断。`physics_attention` 在本地 `Line_0_1` sparse proxy 上改善 global RMSE/coverage，但把 hot q90 和 gradient q90 明显打坏；synthetic 上也没有稳定优势。因此不扩展到 CNN/GCN/Transformer attention、meta-learning 或 A100 broad12/broad21。
- Phase 48 region-preserving inverse-closure acquisition 已关闭为负向诊断。`region_quota_uncertainty` 能把本地 `Line_0_1` hot q90 从 `99.651322` 改到 `60.956245`，但 global RMSE 退化到 `205.832757`、coverage 降到 `0.682105`；`uncertainty_source` 则改善 global/coverage 但伤害 hot/gradient。因此 acquisition 层面仍未找到可 paper-facing 的平衡策略，不上 A100。
- Phase 49 heat-kernel / Green's-function physical feature family 已关闭为 synthetic-positive、AM-Bench-local-negative 诊断。synthetic heat-kernel + region policy 能同时守住自身 global/hot/gradient gate，但本地 `Line_0_1` 仍在 global 与 hot/gradient 间 tradeoff；因此不扩展到 A100 broad12/broad21，下一步应转向显式 moving-source 参数反演。
- Phase 50 explicit nonlinear moving-source parameter inversion 已关闭为 synthetic-positive、AM-Bench-local-negative 诊断。synthetic 能稳定恢复 `start_x/span_x/center_y/sine_y_amp/core_width`，但本地 `Line_0_1` 仅得到 `149.222337 / 133.745276 / 126.979221 / coverage 0.745263`，仍弱于 global 最强 sparse proxy 且 hot q90 不稳。因此不扩展到 A100 broad12/broad21。
- Phase 51 dense-to-sparse source-parameter transfer 已在 A100 上关闭为 synthetic-positive、AM-Bench-dense-negative 诊断。服务器生成 `10,205` 行 denser calibrated `Line_0_1` table；dense 参数拟合改善 global RMSE/coverage，但 `dense_params_sparse_theta` 仍把 hot q90 和 gradient q90 打坏。因此不扩展当前 normalized moving-source grid，下一步转向 scan-strategy XYPT 物理注册 source-path features。
- Phase 52 registered source-path feature gate 已关闭为数据不兼容诊断。A100 检查确认 `AMB2022-03-AMMT-718-Pad_XYPT.h5` 只含 `Xpad/Ypad` scan strategies，目标表却是 `ThermalData/Line_0_1/Signal` single-track camera-pixel table。因此不能把 pad XYPT 作为 `Line_0_1` 的 paper-facing source path；source-inversion broad12/broad21 扩展继续阻塞。
- Phase 53 source-path data pivot gate 已关闭为负向数据门控。`mds2-2716` 有 21 个 `Line_*` thermography groups 和 6 个 pad thermography groups，但 scan strategy 仍只有 `XYPT/Xpad`/`XYPT/Ypad`，且 HDF5 中没有 camera-pixel 到 galvo-mm 注册元数据。`X_pad1/Y_pad1` 的 independent-rescale diagnostic 也未通过 combined global/hot/gradient gate。因此 source-inversion 主线暂停，下一步转回 broad-data process-conditioned route guard 的 paper-facing 贡献整合。
...
- [docs/results/ambench_multiline_process_baseline_guarded_expert_plan_v1.md](docs/results/ambench_multiline_process_baseline_guarded_expert_plan_v1.md): Phase 45 baseline-guarded process expert 方案、Gate 1 prediction-stack probe 基础设施与 A100 focused 命令。
- [docs/results/ambench_bayesian_inverse_closure_pinn_plan_v1.md](docs/results/ambench_bayesian_inverse_closure_pinn_plan_v1.md): Phase 46 Bayesian inverse-closure PINN 方案、本地 synthetic/table sparse validation 结果与不扩展到 A100 的决策。
- [docs/results/ambench_physics_guided_closure_attention_probe_v1.md](docs/results/ambench_physics_guided_closure_attention_probe_v1.md): Phase 47 lightweight physics-guided closure attention/gating probe、本地 synthetic/table 对照结果与不扩展决策。
- [docs/results/ambench_region_preserving_inverse_closure_acquisition_v1.md](docs/results/ambench_region_preserving_inverse_closure_acquisition_v1.md): Phase 48 region-preserving acquisition、conformal calibration、本地 synthetic/table gate 结果与不扩展决策。
- [docs/results/ambench_heat_kernel_inverse_closure_feature_family_v1.md](docs/results/ambench_heat_kernel_inverse_closure_feature_family_v1.md): Phase 49 heat-kernel/Green's-function physical feature family、本地 synthetic/table gate 结果与不扩展决策。
- [docs/results/ambench_moving_source_parameter_inversion_v1.md](docs/results/ambench_moving_source_parameter_inversion_v1.md): Phase 50 explicit nonlinear moving-source parameter inversion、本地 synthetic/table gate 结果与不扩展决策。
- [docs/results/ambench_dense_source_parameter_transfer_v1.md](docs/results/ambench_dense_source_parameter_transfer_v1.md): Phase 51 dense-to-sparse source-parameter transfer、A100 dense `Line_0_1` gate 结果与转向物理注册 source-path features 的决策。
- [docs/results/ambench_registered_source_path_feature_gate_v1.md](docs/results/ambench_registered_source_path_feature_gate_v1.md): Phase 52 registered source-path feature compatibility gate、pad XYPT 与 `Line_0_1` 不兼容结论与 source-inversion 阻塞决策。
- [docs/results/ambench_source_path_data_pivot_gate_v1.md](docs/results/ambench_source_path_data_pivot_gate_v1.md): Phase 53 source-path data inventory、pad rescale diagnostic、source-inversion 阻塞与 process-route pivot 决策。
