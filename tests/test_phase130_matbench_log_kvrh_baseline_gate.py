...
def _load_module():
    script = Path("scripts/server/build_phase130_matbench_log_kvrh_baseline_gate.py")
    spec = importlib.util.spec_from_file_location("phase130_matbench_log_kvrh", script)
...
def _write_payload(path: Path, *, n_rows: int = 90) -> None:
    ...
    payload = {"columns": ["structure", "log10(K_VRH)"], "index": list(range(n_rows)), "data": rows}
    path.parent.mkdir(parents=True, exist_ok=True)
    with gzip.open(path, "wt", encoding="utf-8") as handle:
        json.dump(payload, handle)


def test_phase130_builds_log_kvrh_gate_and_keeps_training_locked(tmp_path: Path):
    module = _load_module()
    raw_path = tmp_path / "raw" / "matbench_log_kvrh.json.gz"
    ...
    manifest = module.build_package(
        ...
        source_url="https://example.invalid/matbench_log_kvrh.json.gz",
        force_download=False,
    )

    gate = manifest["gate"]
    assert manifest["phase"] == 130
    assert gate["status"] in {
        "phase130_matbench_log_kvrh_ready_focused_review",
        "phase130_matbench_log_kvrh_closed_no_stable_guarded_gap",
    }
    assert gate["selected_target"] == "log10_k_vrh"
    assert gate["row_count"] == 90
    assert gate["phase130_model_mechanism_allowed"] is False
    assert gate["phase130_model_training_allowed"] is False
    assert gate["a100_training_allowed_now"] is False
    assert gate["a100_80gb_request_now"] is False
    assert (tmp_path / "out" / "phase130_matbench_log_kvrh_field_table.csv").exists()
    assert (tmp_path / "out" / "phase130_matbench_log_kvrh_gate.json").exists()


def test_phase130_skips_unparseable_structure_rows(tmp_path: Path):
    module = _load_module()
    raw_path = tmp_path / "raw" / "matbench_log_kvrh.json.gz"
...
def test_phase130_rejects_unexpected_payload_columns(tmp_path: Path):
    module = _load_module()
    ...
    try:
        ...
        assert "Unexpected matbench_log_kvrh columns" in str(exc)
    else:
