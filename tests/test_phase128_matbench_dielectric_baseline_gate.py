...
def _load_module():
    script = Path("scripts/server/build_phase128_matbench_dielectric_baseline_gate.py")
    spec = importlib.util.spec_from_file_location("phase128_matbench_dielectric", script)
...
def _write_payload(path: Path, *, n_rows: int = 90) -> None:
    ...
    payload = {"columns": ["structure", "n"], "index": list(range(n_rows)), "data": rows}
    path.parent.mkdir(parents=True, exist_ok=True)
    with gzip.open(path, "wt", encoding="utf-8") as handle:
        json.dump(payload, handle)


def test_phase128_builds_dielectric_gate_and_keeps_training_locked(tmp_path: Path):
    module = _load_module()
    raw_path = tmp_path / "raw" / "matbench_dielectric.json.gz"
    ...
    manifest = module.build_package(
        ...
        source_url="https://example.invalid/matbench_dielectric.json.gz",
        force_download=False,
    )

    gate = manifest["gate"]
    assert manifest["phase"] == 128
    assert gate["status"] in {
        "phase128_matbench_dielectric_n_ready_focused_review",
        "phase128_matbench_dielectric_n_closed_no_stable_guarded_gap",
    }
    assert gate["selected_target"] == "refractive_index_n"
    assert gate["row_count"] == 90
    assert gate["phase128_model_mechanism_allowed"] is False
    assert gate["phase128_model_training_allowed"] is False
    assert gate["a100_training_allowed_now"] is False
    assert gate["a100_80gb_request_now"] is False
    assert (tmp_path / "out" / "phase128_matbench_dielectric_field_table.csv").exists()
    assert (tmp_path / "out" / "phase128_matbench_dielectric_gate.json").exists()


def test_phase128_rejects_unexpected_payload_columns(tmp_path: Path):
    module = _load_module()
    ...
    try:
        ...
        assert "Unexpected matbench_dielectric columns" in str(exc)
    else:
